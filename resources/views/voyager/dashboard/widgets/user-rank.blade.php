    ...
    <div class="panel widget widget-small">
        ...
        <div class="tab-server-rank">
            @foreach($serverInfo as $key => $server)
                <h5 data-url="{{ route('voyager.ranking.toplist') }}" class="server-rank {{ $server['widget'] }}" data-servername="{{ $server['widget'] }}" >{{ strtoupper($key) }}-{{ $server['name'] }}</h5>
            @endforeach
        </div>
        <div class="panel-body rank-list-content-ad">
        </div>
        <div id="rank-loader">
            <img src="{{ asset('images/ajax-loader.gif') }}" alt="rank-loader">
        </div>
    </div>
</div>